...
public class DaoUsuario {
	...
	public LinkedList<Usuario> getAll(){
		...
		try {
			...
			rs= stmt.executeQuery("select id_usuario,nombre,apellido,email,nombre_usuario,tipo_usu from Usuario");
	...
	public Usuario modificarUser(Usuario usu) {
		Usuario u=null;
		PreparedStatement stmt=null;
		ResultSet rs=null;
		try {
			stmt=DbConnector.getInstancia().getConn().prepareStatement(
					"UPDATE usuario SET nombre = ?, apellido = ?, email = ?, nombre_usuario = ?, contraseña = ? WHERE id_usuario = ?"
					);
			rs=stmt.executeQuery();
			if(rs!=null && rs.next()) {
	            ...
	            stmt.setString(4, usu.getNombreUsuario());
